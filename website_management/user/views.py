...
def author_register(request):
    if request.method == 'GET':
        ...
        if not username or not password:
            return render(request, 'user/register.html',
                          {'msg_code': 0, 'msg_info': 'Username or password cannot be empty'})

        if len(username) > 100 or len(password) > 100:
            return render(request, 'user/register.html',
                          {'msg_code': 0, 'msg_info': 'Username or password too long'})

        if password != confirm_password:
            return render(request, 'user/register.html',
                          {'msg_code': 0, 'msg_info': 'Passwords do not match'})

        author = User.objects.filter(username=username)
        if author.exists():
            return render(request, 'user/register.html',
                          {'msg_code': 0, 'msg_info': 'Username already taken'})
...
def author_login(request):
    ...
    if request.method == 'GET':
        ...
        if not username or not password:
            return render(request, 'user/login.html',
                          {'msg_code': -1, 'msg_info': 'Username or password cannot be empty'})
        ...
        try:
            ...
            # 跳转到首页
            return render(request, 'user/index.html',
                          {'msg_code': 0, 'msg_info': 'Login successful'})
        except:
            return render(request, 'user/login.html',
                          {'msg_code': -1, 'msg_info': 'Username or password is incorrect, please try again'})

@require_http_methods(['GET', 'POST'])
def user_re_password(request):
    if request.method == 'GET':
